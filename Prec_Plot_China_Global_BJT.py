...
path_Prec="D:\\Test_data\\2020\\Prec\\Prec_Hourly\\"
...
def get_lon_lat(lon, lat, lons, lats):
   ...
   #确定经纬度格点索引
	...
	#ny =int(ly*(lats-miny)/(maxy-miny))#最小的纬度是第一个索引，绘图时上南下北
	nx=int(lx*(lons-minx)/(maxx-minx))#最小的经度是第一个索引，绘图时上南下北
	return nx, ny
...
for i in fe:
	...
	m.readshapefile('D:\\Test_data\\Map\\basemap\\gadm36_CHN_1', 'states',drawbounds=True)
